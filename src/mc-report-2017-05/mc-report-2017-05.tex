...
Work is on schedule to develop a generalisation of the Charney--Phillips staggering of variables.  The prognostic thermodynamic variable $b_f$ is stored at all cell faces such that $b_f = \theta_f \bm{\hat{g}} \cdot \bm{\hat{n}}_f$ where $f$ is a face, $\theta_f$ is the potential temperature, $\bm{\hat{g}}$ is the unit vector of gravitational acceleration and $\bm{\hat{n}}_f$ is the unit vector that is outward normal to the face.  This arrangement reduces to the traditional Charney--Phillips staggering on Cartesian meshes since $b_f$ is zero for entirely vertical faces and $b_f = \theta_f$ for entirely horizontal faces.

We have formulated a new two-dimensional vertical slice test case that excites the Lorenz computational mode in the model by \citep{weller-shahrokhi2014}.  The new test case is based on the standing waves test by \citet{arakawa-konor1996}, which was designed for a vertically discrete model with horizontally continuous waves \TODO{I still don't understand this model and don't know how best to describe it}.  The test adds grid-scale temperature perturbations to an isothermal atmosphere in hydrostatic balance.

% show diagram with variable placement
% show work-in-progress test results
