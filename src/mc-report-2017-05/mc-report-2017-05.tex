...
\begin{itemize}
	...
	\item Evaluate $\theta$ advection scheme using a variant of the \citet{schaer2002} gravity waves test that incorporates standing waves that excite the Lorenz computational mode
	\item A semi-implicit gravity wave formulation for the Charney--Phillips model
\end{itemize}
}

% - high-order cubicFit
%   - 1D high-order advection on non-uniform meshes
%   - 2D high-order advection on non-uniform meshes (away from boundaries)
...
% three-month extension? -- will need it if we want a third paper
...
% implementation in ExnerFoam with good non-ortho treatment
% standing waves no orography
% non-conforming (block) refinement and conforming refinement
% standing waves with orography

% schaer waves + standing waves
% improved advection scheme (likely needed for good schaer waves solutions)
% semi-implicit gravity waves
