...
My current work focuses on improving the stability and accuracy of a multidimensional advection scheme that is suitable for arbitrary meshes \citep{weller-shahrokhi2014}.  The performance of the advection scheme will be evaluated over steep slopes on terrain-following, cut cell and slanted cell meshes.
In the second monitoring committee report (hereafter referred to as MC2), the slanted cell method was referred to as the cut cell method.  While there are strong similarities between the two methods, the resulting meshes have differences: compare the representation of the same terrain profile using a cut cell mesh (figure~\ref{fig:cutcell-mesh}) with a slanted cell mesh (figure~\ref{fig:slantedcell-mesh}).

The slanted cell method has several advantages over the cut cell method.  First, unlike the construction of cut cell meshes, which can be somewhat involved \citep{hartkopf2011}, since MC2 was written, I have developed a new technique that makes slanted cell meshes straightforward to construct \citep{shaw-weller2016}.  Second, the slanted cell method does not severely constrain the timestep because thin cells are created that are long in the direction of flow.
...
% future work: more stabilisation, 1D tests/comparisons with other schemes, e.g. WENO,

% now say a few words about what I've done to the advection scheme:
...
% timescales?
% stabilise advection scheme on arbitrary 2D meshes
% Charney--Phillips
